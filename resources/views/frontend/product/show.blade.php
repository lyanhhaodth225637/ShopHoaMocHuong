...
@section('content')
    ...
    .product-detail-page .breadcrumb-item a:hover { color: var(--green-dark); }
    .product-detail-page .breadcrumb-item.active { color: var(--text-muted); }
    .product-detail-page .breadcrumb-item+.breadcrumb-item::before { color: #b2e8ea; }
    ...
    .main-viewer:hover .main-viewer__img-wrap img { transform: scale(1.04); }
    ...
    .main-viewer__video-wrap iframe {
        width: 56.25%; /* 9:16 trong khung vuông */
        height: 100%;
        border: none;
        border-radius: 0;
    }
    ...
    .viewer-badge i { font-size: .8rem; color: #ff4444; }
    ...
    .strip-nav-btn.prev { left: 0; }
    .strip-nav-btn.next { right: 0; }
    ...
    .detail-tab-btn:hover:not(.active) { color: var(--text-dark); }
    ...
    .description-content ol { padding-left: 20px; margin-bottom: 14px; }
    ...
    .description-content a { color: var(--green-main); text-decoration: none; }
    .description-content a:hover { color: var(--green-dark); text-decoration: underline; }
    ...
    .related-section .divider-leaf { margin-bottom: 20px; }
    ...
    @media (max-width: 767px) {
        ...
        .detail-tabs-body { padding: 20px 16px; }
        .price-current { font-size: 1.6rem; }
        .thumb-btn { width: 58px; height: 58px; }
        .guarantee-strip { grid-template-columns: repeat(3,1fr); gap: 6px; }
        .cta-row { flex-direction: column; }
        .btn-cart-main { width: 100%; }
        .btn-wishlist { width: 100%; border-radius: 14px; height: 52px; }
    }
    </style>
    ...
    <main class="product-detail-page py-4 pb-5">
        <div class="container">
            ...
            <nav class="mb-4">
                <ol class="breadcrumb">
                    <li class="breadcrumb-item"><a href="{{ route('frontend.home.index') }}"><i class="bi bi-house me-1"></i>Trang chủ</a></li>
                ...
                <div class="col-lg-6 media-col">
                    ...
                    <div class="thumb-strip-wrap">
                        ...
                        <div class="thumb-strip" id="thumbStrip">
                            ...
                            @if($embedUrl)
                                <button type="button" class="thumb-btn thumb-btn--video"
                                    data-type="video"
                                    data-video="{{ $embedUrl }}"
                                    title="Xem video sản phẩm">
                                    {{-- thumbnail YouTube --}}
                                    <img src="https://img.youtube.com/vi/{{ $videoId }}/mqdefault.jpg"
                                         alt="Video" onerror="this.src='{{ $product->main_image ? asset('storage/' . $product->main_image) : asset('images/no-image.png') }}'">
                                </button>
                            @endif

                            {{-- Ảnh chính --}}
                            @if($product->main_image)
                                <button type="button" class="thumb-btn {{ $embedUrl ? '' : 'active' }}"
                                    data-type="image"
                ...
                <div class="detail-tabs-body">
                    ...
                    <div class="tab-pane-content" id="tab-spec">
                        <table class="table table-sm" style="font-size:.875rem;">
                            <tbody>
                                <tr><td style="color:var(--text-muted);width:150px;">Mã sản phẩm</td><td class="fw-600">{{ $product->sku ?? '—' }}</td></tr>
                                <tr><td style="color:var(--text-muted);">Tồn kho</td><td class="fw-600">{{ $product->stock_quantity }} sản phẩm</td></tr>
                                @foreach($product->categories as $cat)
                                    <tr><td style="color:var(--text-muted);">Danh mục</td><td class="fw-600">{{ $cat->name }}</td></tr>
                                @endforeach
                            </tbody>
                        </table>
                    </div>
                                ...
                                <div class="rel-card h-100" style="display:flex;flex-direction:column;">
                                    ...
                                    <div class="rel-card__body" style="flex:1;display:flex;flex-direction:column;">
                                        ...
                                        <form class="ajax-add-cart-form"
                                            ...
                                            <button type="submit" class="btn-add-cart" style="border-radius:10px;padding:9px 0;font-size:.82rem;">
